...
describe("First check the Element Is present in dom or not", () => {
  ...
  test("Random Button is Disabled after Click", () => {
    render(<Home handelget={gettodos} />)
    const Randombutton = screen.getByText("Random Id")
    expect(Randombutton).not.toBeDisabled()
    fireEvent.click(Randombutton)
    expect(Randombutton).toBeDisabled()
    expect(Randombutton.innerHTML).toBe("Loading")
  })
  test("Random Button is change text after click ", () => {
    render(<Home handelget={gettodos} />)
    const Randombutton = screen.getByText("Random Id")
    expect(Randombutton.innerHTML).toBe("Random Id")
    fireEvent.click(Randombutton)
    expect(Randombutton.innerHTML).toBe("Loading")
   })
   })
